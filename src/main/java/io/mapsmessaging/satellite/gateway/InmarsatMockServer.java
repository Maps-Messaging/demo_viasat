...
package io.mapsmessaging.satellite.gateway;
...
public class InmarsatMockServer {
  ...
  private final String base = "/iotMessaging/v1";
  ...
  private void handleSubmitMobileTerminated(HttpExchange exchange, String requestId, long startNanos) throws IOException {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (hasPayload) {
        try {
          Base64.getDecoder().decode(payloadRaw);
          base64Ok = true;
      ...
      outgoingMessages.add(Base64.getDecoder().decode(payloadRaw));
    }
  ...
  private void handleMobileOriginated(HttpExchange exchange, String reqId, long t0) throws IOException {
    ...
    for (byte[] payload; (payload = incomingMessages.poll()) != null; ) {
      ...
      String messageUtc = Instant.now().minusSeconds(1).toString();
      String receiveUtc = Instant.now().toString();
    ...
    String nextStartTime = Instant.now().toString();
